...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/export-icons.js");
/******/ })
/************************************************************************/
/******/ ({
...
module.exports = "file://" + String(context.scriptPath).split(".sketchplugin/Contents/Sketch")[0] + ".sketchplugin/Contents/Resources/_webpack_resources/985a1a1ca2f7fbf8c2a0afbd7652df67.html";
...
/* harmony import */ var _skpm_dialog__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! @skpm/dialog */ "./node_modules/@skpm/dialog/lib/index.js");
/* harmony import */ var _skpm_dialog__WEBPACK_IMPORTED_MODULE_3___default = /*#__PURE__*/__webpack_require__.n(_skpm_dialog__WEBPACK_IMPORTED_MODULE_3__);
...
var webviewIdentifier = 'icontocode.webview';
/* harmony default export */ __webpack_exports__["default"] = (function () {
  ...
  webContents.on('openUrl', function (url) {
  ...
  webContents.on('did-finish-load', function () {
    ...
    webContents.executeJavaScript("setInit(".concat(JSON.stringify({
      theme: theme
    }), ")")).catch(console.error);
  ...
  webContents.on('getZip', function (data) {
    ...
    fetch(url, options).then(function (resp) {
      ...
      var exportPath = '';
      var selected = _skpm_dialog__WEBPACK_IMPORTED_MODULE_3___default.a.showOpenDialogSync({
      ...
      if (!selected.length) {
        webContents.executeJavaScript("finishExport()").catch(console.error);
        return;
      } else {
...
var getWindow = function getWindow() {
  return sketch_module_web_view__WEBPACK_IMPORTED_MODULE_1___default.a.fromId(webviewIdentifier);
};

function onSelection(context) {
  var segments = getSelectedItems();
  var browserWindow = getWindow();
  var webContents = browserWindow.webContents;
  webContents.executeJavaScript("setData(".concat(JSON.stringify(segments), ")")).catch(console.error);
}

function onSelectionChanged(context) {
  try {
    onSelection(context);
  } catch (e) {//console.log('SELECTION ERROR', e.message, e.stack)
  }
}
...
var getSelectedItems = function getSelectedItems() {
  ...
  var selectedPage = document.pages.find(function (page) {
    return page.selected;
  });
  var exportWholePage = selection.layers.length === 0;
  var segments = [];
  selectedPage.layers.forEach(function (item) {
    if (exportWholePage || item.selected) {
      var segmentName = selectedPage.name;
      var icon = exportIcon(item);
      var existingSegment = segments.findIndex(function (seg) {
        return seg.name === segmentName;
      });

      if (existingSegment !== -1) {
        var lastIconListIndex = segments[existingSegment].icons.length - 1;
        var lastIconList = segments[existingSegment].icons[lastIconListIndex];
...
var exportIcon = function exportIcon(layer) {
  if (layer.type === 'Artboard' || layer.type === 'Group' || layer.type === 'Shape') {
    ...
    var svgFile = sketch__WEBPACK_IMPORTED_MODULE_0___default.a.export(icon, options);
    return {
      'svg': svgFile,
      'name': layer.name
    };
...
//# sourceMappingURL=__export-icons.js.map